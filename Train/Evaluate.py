...
def load_model(checkpoint_path: str, device: torch.device) -> ChestXRayMultiTask:
    ...
    # Debug : affiche les 5 premières clés du checkpoint pour vérifier
    print("Clés du checkpoint (5 premières):", list(saved_state.keys())[:5])
    print("Clés du modèle (5 premières):", list(model.state_dict().keys())[:5])
    
    # Remapping : checkpoint sans "encoder.model." → modèle qui attend "encoder.model."
    model_keys = set(model.state_dict().keys())
    new_state = {}
    for k, v in saved_state.items():
        if k in model_keys:
            new_state[k] = v
        elif k.startswith("encoder.") and not k.startswith("encoder.model."):
            new_k = "encoder.model." + k[len("encoder."):]
            if new_k in model_keys:
                new_state[new_k] = v
            else:
                new_state[k] = v  # garde l'original
        else:
            new_state[k] = v
    ...
    if missing:
        print(f"⚠ Clés manquantes ({len(missing)}) : {missing[:3]}{'...' if len(missing) > 3 else ''}")
    if unexpected:
        print(f"⚠ Clés inattendues ({len(unexpected)}) : {unexpected[:3]}{'...' if len(unexpected) > 3 else ''}")
    if not missing and not unexpected:
        print("✓ Tous les poids chargés parfaitement")
