...
# ---------------------------------------------------------------------------
# Chargement d'images depuis le disque local (LIDC + NIH)
# ---------------------------------------------------------------------------
import os
import pandas as pd
import numpy as np
...
def _load_png(path: str) -> torch.Tensor:
    ...
    # Si grayscale -> duplique en 3 canaux (RadDino attend généralement 3 canaux)
    if arr.ndim == 2:
...
class LungMultiSourceDataset(Dataset):
    ...
    def __init__(
        ...
        df_loc["_filename"] = df_loc[loc_file_col].astype(str)
        ...
        df_cls = df_cls.copy()
        df_cls["_dataset"] = "nih"
        df_cls["_filename"] = df_cls[cls_file_col].astype(str)
        df_cls["_label_col"] = cls_label_col

        # Concat
        self.df = pd.concat([df_loc, df_cls], ignore_index=True)
...
if __name__ == "__main__":
    ...
    def train_one_epoch(epoch: int) -> dict:
        ...
        for images, labels, coords_batch, meta in train_loader:
            images = images.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True).float()
            target_heatmaps = heatmap_generator.batch_generate(coords_batch, device)
    ...
    # --------------------------------------------------------------------
    best_val = float("inf")
    patience = 5  # peu de patience (early stopping fort)
    bad_epochs = 0
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch == 1 or epoch % 5 == 0:
            ...
            print(
                ...
                f"train loss={metrics['loss']:.4f} (cls={metrics['loss_cls']:.4f}, loc={metrics['loss_loc']:.4f}) | "
                f"val loss={val_metrics['val_loss']:.4f} (cls={val_metrics['val_loss_cls']:.4f}, loc={val_metrics['val_loss_loc']:.4f}) | "
                f"lrs={lrs} | bad_epochs={bad_epochs}/{patience}"
            )
